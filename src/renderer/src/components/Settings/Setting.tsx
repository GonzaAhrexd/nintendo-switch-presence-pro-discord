      ...
      <div className="mb-6">
        ...
        <div className="flex gap-2">
          {t.languages.map((lang) => (
            <button
              key={lang.code}
              className={`px-3 py-1 rounded transition-colors border ${inputBorder} text-sm font-medium focus:outline-none focus:ring-2 focus:ring-red-400 ${
      ...
      <div className={`flex items-center justify-between py-2 border-t ${divider}`}>
        ...
        <button
          className={`w-12 h-6 flex items-center rounded-full p-1 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-red-400 ${
            pendingDarkMode ? 'bg-red-600' : (darkMode ? 'bg-zinc-700' : 'bg-zinc-300')
          }`}
          onClick={() => setPendingDarkMode((v) => !v)}
          aria-pressed={pendingDarkMode}
        >
          <span
            className={`w-5 h-5 ${darkMode ? 'bg-zinc-900' : 'bg-white'} rounded-full shadow transform transition-transform duration-200 ${
